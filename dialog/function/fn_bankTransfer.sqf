...
if(_val > pbh_life_atmcash) exitWith {hint localize "STR_ATM_NotEnough"};
_tax = [_val] call life_fnc_taxRate;
if((_val + _tax) > pbh_life_atmcash) exitWith {hint format[localize "STR_ATM_SentMoneyFail",_val,_tax]};
