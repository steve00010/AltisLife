...
//-------------------- 1. FIND POSITION
	...
	while {!_accepted} do {
		...
		if ((_flatPos distance (getMarkerPos "respawn_west")) > 1000 && (_flatPos distance (getMarkerPos currentAO)) > 800) then
		{
			_accepted = true;
		};
	};
...
//-------------------- 2. SPAWN OBJECTIVES
	...
	priorityObj1 = "O_Mortar_01_F" createVehicle _flatPos1;
	waitUntil {!isNull priorityObj1};
	priorityObj1 setDir _PTdir;
	
	sleep 0.3;
	
	priorityObj2 = "O_Mortar_01_F" createVehicle _flatPos2;
...
//-------------------- 7. BRIEF
	...
	priorityTargetText = "Mortar Team"; publicVariable "priorityTargetText";
	"priorityMarker" setMarkerText "Priority Target: Mortar Team"; publicVariable "priorityMarker";

	_briefing = "<t align='center' size='2.2'>Priority Target</t><br/><t size='1.5' color='#b60000'>Mortar Team</t><br/>____________________<br/>OPFOR forces are setting up a mortar position to hit you guys damned hard! We've picked up their positions with thermal imaging scans and have marked it on your map.<br/><br/>This is a priority target, boys! They're just setting up now; they'll be firing in about five minutes!";
	GlobalHint = _briefing; hint parseText _briefing; publicVariable "GlobalHint";
	showNotification = ["NewPriorityTarget", "Eliminate Mortar Team"]; publicVariable "showNotification";
	...
	_firingMessages = [
		...
		"The Mortar team's are firing, boys! Down on the ground!",
...
while { canMove priorityObj1 || canMove priorityObj2 } do {
		...
		while {!_accepted} do {
			...
			if (!isNull _unit) then {
				...
				if ((_targetPos distance (getMarkerPos "respawn_west")) > 600 && vehicle _unit == _unit && side _unit == WEST) then { 
					_accepted = true; 
				} else {
					sleep 7;																// default 10
				};
