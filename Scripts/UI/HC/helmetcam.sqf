	...
	if ( !( (goggles player) in hcam_goggles ) ) then {
		...
		if (_camOn) then 
		{
			...
			if (hcam_target != _veh) then 
			{
				...
				_eyepitch = (asin((eyeDirection hcam_target) select 2));
				
				hcam_cam setposasl [(eyepos hcam_target select 0)+0.075*sin(getDir hcam_target), (eyepos hcam_target select 1)+0.075*cos(getDir hcam_target), (eyepos hcam_target select 2)-0.075];;
				_target setposasl [(eyepos hcam_target select 0)+10*sin(getDir hcam_target), (eyepos hcam_target select 1)+10*cos(getDir hcam_target), ((eyepos hcam_target select 2)-0.075)+_eyepitch];
			} 
			else
			{
