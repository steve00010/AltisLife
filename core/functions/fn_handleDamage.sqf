...
if(!isNull _source) then 
{
	if(_source != _unit) then 
	{
		...
		_curWep = currentWeapon _source;
		if(_projectile in ["B_9x21_Ball","30Rnd_556x45_Stanag_Tracer_Red"] && _curWep in ["hgun_P07_snds_F","arifle_TRG20_F"]) then {
			if(side _source == west && playerSide != west) then {
				private["_distance","_isVehicle","_isQuad"];
				_distance = if(_projectile == "30Rnd_556x45_Stanag_Tracer_Red") then {100} else {35};
