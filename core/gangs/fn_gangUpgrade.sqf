...
if(_action) then {
	if(pbh_life_atmcash < _upgradePrice) exitWith {
		hint parseText format[
			...
			[pbh_life_atmcash] call life_fnc_numberText,
			[_upgradePrice - pbh_life_atmcash] call life_fnc_numberText
		];
	};
	__SUB__(pbh_life_atmcash,_upgradePrice);
