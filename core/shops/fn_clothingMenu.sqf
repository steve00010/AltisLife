...
if((_this select 3) in ["cop","swat"] && playerSide != west) exitWith {hint "You need to be a Cop to use this store!"; closeDialog 0;};
...
{
	//diag_log format["Check to see if license is needed : %1", [_var select 0] call life_fnc_varToStr];
	if(!(missionNamespace getVariable (_var select 0))) exitWith 
	{
		//diag_log "License Required so exit.";
		_exitshop = true;
...
if (!_exitshop) then
{
	...
	if(isNil "life_clothesPurchased") exitWith
	{
		...
		if(life_oldClothes != "") then {player addUniform life_oldClothes; [] call life_fnc_customUniforms;} else {removeUniform player};
	...
	{
		if(life_oldClothes != uniform player) then {player addUniform life_oldClothes; [] call life_fnc_customUniforms;};
	};
	//Check hat
