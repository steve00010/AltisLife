...
#define VEH_TYPE "O_APC_Tracked_02_cannon_F","O_APC_Wheeled_02_rcws_F","O_APC_Tracked_02_cannon_F","I_APC_Wheeled_03_cannon_F","I_APC_tracked_03_cannon_F"
#define AIR_TYPE "I_Heli_light_03_F","O_Heli_Light_02_F","O_Heli_Light_02_F"
#define NAVAL_TYPE "O_Boat_Armed_01_hmg_F","I_Boat_Armed_01_minigun_F"
#define STATIC_TYPE "O_HMG_01_F","O_HMG_01_high_F","O_Mortar_01_F"

private ["_enemiesArray","_randomPos","_patrolGroup","_AOvehGroup","_AOveh","_AOnavalGroup","_AOnaval","_AOmrapGroup","_AOmrap","_pos","_spawnPos","_overwatchGroup","_x","_staticGroup","_static","_aaGroup","_aa","_airGroup","_air","_sniperGroup","_staticDir"];
...
for "_x" from 0 to (1 + (random 2)) do {
	_AOnavalGroup = createGroup east;
	_randomPos = [[[getMarkerPos currentAO, PARAMS_AOSize],[]],["water","out"]] call BIS_fnc_randomPos;
	_AOnaval = [NAVAL_TYPE] call BIS_fnc_selectRandom createVehicle [_randomPos select 0,_randomPos select 1,1000];
	waitUntil{!isNull _AOnaval};
	if (random 1 >= 0.25) then {
		_AOnaval allowCrewInImmobile true;
	};
		"O_diver_F" createUnit [_randomPos,_AOnavalGroup];
		"O_diver_F" createUnit [_randomPos,_AOnavalGroup];
		"O_diver_F" createUnit [_randomPos,_AOnavalGroup];
		((units _AOnavalGroup) select 0) assignAsDriver _AOnaval;
		((units _AOnavalGroup) select 0) moveInDriver _AOnaval;
		((units _AOnavalGroup) select 1) assignAsGunner _AOnaval;
		((units _AOnavalGroup) select 1) moveInGunner _AOnaval;
		((units _AOnavalGroup) select 2) assignAsCommander _AOnaval;
		((units _AOnavalGroup) select 2) moveInCommander _AOnaval;
	[_AOnavalGroup, getMarkerPos currentAO, 600] call BIS_fnc_taskPatrol;
	_AOnaval lock 3;
	
	_enemiesArray = _enemiesArray + [_AOnavalGroup,_AOnaval];
	sleep 0.1;
	_enemiesArray = _enemiesArray + [_AOnaval];

	{
		_x addCuratorEditableObjects [[_AOnaval], false];
		_x addCuratorEditableObjects [units _AOnavalGroup, false];
	} foreach adminCurators;
...
//[(units _AOnavalGroup)] call QS_fnc_setSkill2;
